...
public class ButtonBacklightBrightness extends CustomDialogPref<AlertDialog>
  ...
  public ButtonBacklightBrightness(Context context, AttributeSet attrs) {
    ...
    if (DeviceUtils.hasKeyboardBacklightSupport(context)) {
      final boolean isSingleValue =
          KEYBOARD_BRIGHTNESS_TOGGLE_MODE_ONLY
              == context
                  .getResources()
                  .getInteger(
      ...
      mKeyboardBrightness =
          new BrightnessControl(LineageSettings.Secure.KEYBOARD_BRIGHTNESS, isSingleValue);
      mActiveControl = mKeyboardBrightness;
    }
    if (DeviceUtils.hasButtonBacklightSupport(context)) {
      final boolean isSingleValue =
          BUTTON_BRIGHTNESS_TOGGLE_MODE_ONLY
              == context
                  .getResources()
                  .getInteger(
                      org.lineageos.platform.internal.R.integer
  ...
  private String getTimeoutString(int timeout) {
    return getContext()
        .getResources()
        .getQuantityString(R.plurals.backlight_timeout_time, timeout, timeout);
  }
  ...
  private int getTimeout() {
    return LineageSettings.Secure.getInt(
            ...
            DEFAULT_BUTTON_TIMEOUT * 1000)
        / 1000;
  }

  private void applyTimeout(int timeout) {
    LineageSettings.Secure.putInt(
        mResolver, LineageSettings.Secure.BUTTON_BACKLIGHT_TIMEOUT, timeout * 1000);
  }
  ...
  private void updateTimeoutEnabledState() {
    float buttonBrightness =
        mButtonBrightness != null ? mButtonBrightness.getBrightness(false) : 0.0f;
  ...
  private class BrightnessControl
      implements SeekBar.OnSeekBarChangeListener, CheckBox.OnCheckedChangeListener {
  ...
  private class ButtonBrightnessControl extends BrightnessControl {
    ...
    public ButtonBrightnessControl(
        String brightnessSetting,
        String onlyWhenPressedSetting,
        boolean singleValue,
        float defaultBrightness) {
